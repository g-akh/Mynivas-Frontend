  ...
  return (
    <View style={[s.card, !isVacant && s.cardOccupied]}>
      <View style={s.cardHeader}>
        ...
        <View style={[s.statusChip, { backgroundColor: isVacant ? theme.colors.success + "15" : theme.colors.danger + "15", borderColor: isVacant ? theme.colors.success + "44" : theme.colors.danger + "44" }]}>
          <Text style={{ fontSize: 11, fontWeight: "700", color: isVacant ? theme.colors.success : theme.colors.danger }}>
            {isVacant ? "Vacant" : "Occupied"}
          </Text>
        </View>
        <TouchableOpacity style={{ padding: 4 }} onPress={handleDelete}>
          <MaterialIcons name="delete-outline" size={16} color={theme.colors.textDisabled} />
        </TouchableOpacity>
      </View>
      ...
      {isOccupied && (
        <View style={s.occupantRow}>
          ...
          {slot.vehicle_num ? (
            <>
              <MaterialIcons name="directions-car" size={12} color={theme.colors.textSecondary} style={{ marginLeft: 8 }} />
      ...
      <View style={s.actionRow}>
        {isVacant ? (
          <TouchableOpacity style={[s.actionBtn, s.occupyBtn]} onPress={() => setShowOccupyModal(true)} disabled={isPending}>
            ...
            <Text style={[s.actionText, { color: theme.colors.primary }]}>Mark Occupied</Text>
          </TouchableOpacity>
        ) : (
          <TouchableOpacity style={[s.actionBtn, s.vacateBtn]} onPress={handleVacate} disabled={isPending}>
            ...
            <Text style={[s.actionText, { color: theme.colors.success }]}>Mark Vacant</Text>
          </TouchableOpacity>
        )}
      </View>
      ...
      <Modal visible={showOccupyModal} transparent animationType="slide">
        <TouchableOpacity style={s.overlay} activeOpacity={1} onPress={() => setShowOccupyModal(false)}>
          <View style={s.sheet}>
            ...
            <TextInput style={s.input} value={occupant} onChangeText={setOccupant} placeholder="Who is parking here?" placeholderTextColor={theme.colors.textDisabled} />
            <Text style={s.fieldLabel}>Vehicle Number (optional)</Text>
            <TextInput style={s.input} value={vehicleNum} onChangeText={setVehicleNum} placeholder="e.g. KA01AB1234" autoCapitalize="characters" placeholderTextColor={theme.colors.textDisabled} />
            <LoadingButton title="Confirm" loadingTitle="Saving…" onPress={handleOccupy} isLoading={isPending} style={{ marginTop: theme.spacing.lg }} />
    ...
    <Modal visible={visible} transparent animationType="slide">
      <TouchableOpacity style={s.overlay} activeOpacity={1} onPress={onClose}>
        <View style={s.sheet}>
          ...
          <TextInput style={s.input} value={slotNumber} onChangeText={setSlotNumber} placeholder="e.g. A-01 or P1" autoCapitalize="characters" placeholderTextColor={theme.colors.textDisabled} />
          <LoadingButton title="Add Slot" loadingTitle="Adding…" onPress={() => { if (!slotNumber.trim()) { showToast({ type: "error", message: "Slot number required" }); return; } mutate(); }} isLoading={isPending} style={{ marginTop: theme.spacing.lg }} />
...
export default function ParkingScreen() {
  ...
  return (
    <SafeAreaView style={s.safe} edges={["top"]}>
      <AppHeader title="Parking" />

      <View style={s.countsBar}>
        <View style={s.countItem}>
          <Text style={[s.countNum, { color: theme.colors.success }]}>{vacant}</Text>
          <Text style={s.countLabel}>Vacant</Text>
        </View>
        <View style={s.countSep} />
        <View style={s.countItem}>
          <Text style={[s.countNum, { color: theme.colors.danger }]}>{occupied}</Text>
          <Text style={s.countLabel}>Occupied</Text>
        </View>
        <View style={s.countSep} />
        <View style={s.countItem}>
          <Text style={[s.countNum, { color: theme.colors.textPrimary }]}>{slots.length}</Text>
...
const s = StyleSheet.create({
  ...
  cardOccupied: { borderColor: theme.colors.danger + "44" },
  ...
  slotNum: { fontSize: theme.fontSize.md, fontWeight: theme.fontWeight.semibold, color: theme.colors.textPrimary },
  slotType: { fontSize: theme.fontSize.xs, color: theme.colors.textSecondary },
  statusChip: { paddingHorizontal: 8, paddingVertical: 3, borderRadius: theme.borderRadius.full, borderWidth: 1, marginRight: 6 },
  occupantRow: { flexDirection: "row", alignItems: "center", gap: 4, marginBottom: theme.spacing.sm },
  occupantText: { fontSize: 12, color: theme.colors.textSecondary },
  actionRow: { flexDirection: "row" },
  actionBtn: { flex: 1, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 5, paddingVertical: 9, borderRadius: theme.borderRadius.md, borderWidth: 1 },
  occupyBtn: { borderColor: theme.colors.primary + "55", backgroundColor: theme.colors.primary + "10" },
  vacateBtn: { borderColor: theme.colors.success + "55", backgroundColor: theme.colors.success + "10" },
  actionText: { fontSize: theme.fontSize.sm, fontWeight: "600" },
  fab: { position: "absolute", bottom: 28, right: 20, width: 56, height: 56, borderRadius: 28, backgroundColor: "#3498DB", justifyContent: "center", alignItems: "center", ...theme.shadow.lg },
